import type { Color, Emoji, KudoStyle, Profile, User } from "@prisma/client";
...
export const loader: LoaderFunction = async ({ request, params }) => {
  ...
  const user: Pick<User, "id" | "email" | "profile"> | null = await getUser(
    request
  );
  const recipient: User | null = await getUserById(userId);
...
export default function KudoModal() {
  ...
  const { user, recipient } = useLoaderData() as DataType;
      ...
      <form method="post">
        ...
        <div className="flex flex-col md:flex-row gap-y-2 md:gap-y-0">
          <div className="text-center flex flex-col items-center gap-y-2 pr-8">
            <UserCircle
              profile={recipient?.profile as unknown as Profile}
              className="h-24 w-24"
            />
          ...
          <div className="flex-1 flex flex-col gap-y-4">
            <textarea
              ...
              placeholder={`Shower praises on ${recipient?.profile.firstName}...`}
            />
        ...
        <div className="flex flex-col items-center md:flex-row gap-x-24 gap-y-2 md:gap-y-0">
          <Kudo profile={user?.profile ?? null} kudo={formData} />
